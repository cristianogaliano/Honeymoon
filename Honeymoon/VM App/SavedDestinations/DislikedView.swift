...
struct DislikedView: View {
    ...
    var body: some View {
            ...
            if !isGridViewActive {
                List {
                    ForEach(destinations, id: \.id) { destination in
                        if savedPreferences[destination.place]?["like"] == false {
                            NavigationLink(
                                    destination: DestinationPromotionsView(destination: destination),
                                    label: {
                                        DestinationListView(like: false, destination: destination)
                                    })
                        }
                        ...
                        ForEach(destinations, id: \.id) { destination in
                            if savedPreferences[destination.place]?["like"] == false {
                                NavigationLink(
                                    destination: DestinationPromotionsView(destination: destination),
                                    label: {
                                        DestinationGridItemView(like: false, destination: destination, columns: gridColumn, city: destination.place, country: destination.country)
                                    })
                            }
                        }
        ...
        .navigationTitle(Text("❌ I DID NOT LIKE..."))
...
struct DislikedView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        DislikedView(savedPreferences: demoPreferences, destinations: demoDestinations)
